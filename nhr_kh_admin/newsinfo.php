...
class cnews extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('news', 'news', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// brief
		$this->brief = new cField('news', 'news', 'x_brief', 'brief', '`brief`', '`brief`', 201, -1, FALSE, '`brief`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['brief'] = &$this->brief;

		// details
		$this->details = new cField('news', 'news', 'x_details', 'details', '`details`', '`details`', 201, -1, FALSE, '`details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['details'] = &$this->details;

		// pic
		$this->pic = new cField('news', 'news', 'x_pic', 'pic', '`pic`', '`pic`', 200, -1, TRUE, '`pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['pic'] = &$this->pic;

		// key_wrds
		$this->key_wrds = new cField('news', 'news', 'x_key_wrds', 'key_wrds', '`key_wrds`', '`key_wrds`', 201, -1, FALSE, '`key_wrds`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->brief->Exportable) $Doc->ExportCaption($this->brief);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->brief->Exportable) $Doc->ExportField($this->brief);
