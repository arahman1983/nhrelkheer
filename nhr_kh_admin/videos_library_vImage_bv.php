...
$videos_library_vImage_blobview = NULL; // Initialize page object first

class cvideos_library_vImage_blobview extends cvideos_library {
	...
	// Page object name
	var $PageObjName = 'videos_library_vImage_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'videos_library');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('videos_library', 'x_vImage');
		...
		// SQL constructor in videos_library class, videos_libraryinfo.php
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('vImage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('vImage')));
				if (trim(strval($this->Recordset->fields('vImage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('vImage') . "\"");
				}
...
// Create page object
if (!isset($videos_library_vImage_blobview)) $videos_library_vImage_blobview = new cvideos_library_vImage_blobview();

// Page init
$videos_library_vImage_blobview->Page_Init();

// Page main
$videos_library_vImage_blobview->Page_Main();
?>
<?php
$videos_library_vImage_blobview->Page_Terminate();
?>
