...
$sectors_sector_pic_blobview = NULL; // Initialize page object first

class csectors_sector_pic_blobview extends csectors {
	...
	// Page object name
	var $PageObjName = 'sectors_sector_pic_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('sectors', 'x_sector_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('sector_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('sector_pic')));
				if (trim(strval($this->Recordset->fields('sector_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('sector_pic') . "\"");
				}
...
// Create page object
if (!isset($sectors_sector_pic_blobview)) $sectors_sector_pic_blobview = new csectors_sector_pic_blobview();

// Page init
$sectors_sector_pic_blobview->Page_Init();

// Page main
$sectors_sector_pic_blobview->Page_Main();
?>
<?php
$sectors_sector_pic_blobview->Page_Terminate();
?>
