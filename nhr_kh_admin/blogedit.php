	...
	function RenderRow() {
		...
		$this->key_wrds->ViewCustomAttributes = "";
			...
			if ($this->pic->UseColorbox) {
				...
				$this->pic->LinkAttrs["data-rel"] = "blog_x_pic";
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $blog->title->FldCaption(), $blog->title->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_breif");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $blog->breif->FldCaption(), $blog->breif->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_details");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $blog->details->FldCaption(), $blog->details->ReqErrMsg)) ?>");
			felm = this.GetElements("x" + infix + "_pic");
			elm = this.GetElements("fn_x" + infix + "_pic");
			if (felm && elm && !ew_HasValue(elm))
				return this.OnError(felm, "<?php echo ew_JsEncode2(str_replace("%s", $blog->pic->FldCaption(), $blog->pic->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_key_wrds");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $blog->key_wrds->FldCaption(), $blog->key_wrds->ReqErrMsg)) ?>");
...
<?php if ($blog->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_blog_id" class="col-sm-2 control-label ewLabel"><?php echo $blog->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $blog->id->CellAttributes() ?>>
<span id="el_blog_id">
...
<?php if ($blog->title->Visible) { // title ?>
	<div id="r_title" class="form-group">
		<label id="elh_blog_title" for="x_title" class="col-sm-2 control-label ewLabel"><?php echo $blog->title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $blog->title->CellAttributes() ?>>
<span id="el_blog_title">
<input type="text" data-table="blog" data-field="x_title" name="x_title" id="x_title" size="70" maxlength="150" placeholder="<?php echo ew_HtmlEncode($blog->title->getPlaceHolder()) ?>" value="<?php echo $blog->title->EditValue ?>"<?php echo $blog->title->EditAttributes() ?>>
</span>
<?php echo $blog->title->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($blog->breif->Visible) { // breif ?>
	<div id="r_breif" class="form-group">
		<label id="elh_blog_breif" for="x_breif" class="col-sm-2 control-label ewLabel"><?php echo $blog->breif->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $blog->breif->CellAttributes() ?>>
<span id="el_blog_breif">
<input type="text" data-table="blog" data-field="x_breif" name="x_breif" id="x_breif" size="70" placeholder="<?php echo ew_HtmlEncode($blog->breif->getPlaceHolder()) ?>" value="<?php echo $blog->breif->EditValue ?>"<?php echo $blog->breif->EditAttributes() ?>>
</span>
<?php echo $blog->breif->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($blog->details->Visible) { // details ?>
	<div id="r_details" class="form-group">
		<label id="elh_blog_details" for="x_details" class="col-sm-2 control-label ewLabel"><?php echo $blog->details->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $blog->details->CellAttributes() ?>>
<span id="el_blog_details">
<textarea data-table="blog" data-field="x_details" name="x_details" id="x_details" cols="75" rows="4" placeholder="<?php echo ew_HtmlEncode($blog->details->getPlaceHolder()) ?>"<?php echo $blog->details->EditAttributes() ?>><?php echo $blog->details->EditValue ?></textarea>
</span>
<?php echo $blog->details->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($blog->pic->Visible) { // pic ?>
	<div id="r_pic" class="form-group">
		<label id="elh_blog_pic" class="col-sm-2 control-label ewLabel"><?php echo $blog->pic->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $blog->pic->CellAttributes() ?>>
<span id="el_blog_pic">
<div id="fd_x_pic">
<span title="<?php echo $blog->pic->FldTitle() ? $blog->pic->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($blog->pic->ReadOnly || $blog->pic->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
	<input type="file" title=" " data-table="blog" data-field="x_pic" name="x_pic" id="x_pic"<?php echo $blog->pic->EditAttributes() ?>>
</span>
<input type="hidden" name="fn_x_pic" id= "fn_x_pic" value="<?php echo $blog->pic->Upload->FileName ?>">
...
<input type="hidden" name="fx_x_pic" id= "fx_x_pic" value="<?php echo $blog->pic->UploadAllowedFileExt ?>">
<input type="hidden" name="fm_x_pic" id= "fm_x_pic" value="<?php echo $blog->pic->UploadMaxFileSize ?>">
</div>
<table id="ft_x_pic" class="table table-condensed pull-left ewUploadTable"><tbody class="files"></tbody></table>
</span>
<?php echo $blog->pic->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($blog->key_wrds->Visible) { // key_wrds ?>
	<div id="r_key_wrds" class="form-group">
		<label id="elh_blog_key_wrds" for="x_key_wrds" class="col-sm-2 control-label ewLabel"><?php echo $blog->key_wrds->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $blog->key_wrds->CellAttributes() ?>>
<span id="el_blog_key_wrds">
<input type="text" data-table="blog" data-field="x_key_wrds" name="x_key_wrds" id="x_key_wrds" size="70" placeholder="<?php echo ew_HtmlEncode($blog->key_wrds->getPlaceHolder()) ?>" value="<?php echo $blog->key_wrds->EditValue ?>"<?php echo $blog->key_wrds->EditAttributes() ?>>
</span>
<?php echo $blog->key_wrds->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
