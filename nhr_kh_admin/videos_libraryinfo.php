...
//
// Table class for videos_library
//
class cvideos_library extends cTable {
	var $vid;
	var $vTitle;
	var $vImage;
	var $vUrl;
	...
	function __construct() {
		...
		$this->TableVar = 'videos_library';
		$this->TableName = 'videos_library';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`videos_library`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// vid
		$this->vid = new cField('videos_library', 'videos_library', 'x_vid', 'vid', '`vid`', '`vid`', 3, -1, FALSE, '`vid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->vid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['vid'] = &$this->vid;

		// vTitle
		$this->vTitle = new cField('videos_library', 'videos_library', 'x_vTitle', 'vTitle', '`vTitle`', '`vTitle`', 200, -1, FALSE, '`vTitle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['vTitle'] = &$this->vTitle;

		// vImage
		$this->vImage = new cField('videos_library', 'videos_library', 'x_vImage', 'vImage', '`vImage`', '`vImage`', 200, -1, TRUE, '`vImage`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['vImage'] = &$this->vImage;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`videos_library`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('vid', $rs))
				ew_AddFilter($where, ew_QuotedName('vid', $this->DBID) . '=' . ew_QuotedValue($rs['vid'], $this->vid->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->vid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@vid@", ew_AdjustSql($this->vid->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("videos_libraryview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("videos_libraryview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "videos_libraryadd.php?" . $this->UrlParm($parm);
		else
			$url = "videos_libraryadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("videos_libraryedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("videos_libraryadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("videos_librarydelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "vid:" . ew_VarToJson($this->vid->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->vid->CurrentValue)) {
			$sUrl .= "vid=" . urlencode($this->vid->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["vid"]) : ew_StripSlashes(@$_GET["vid"]); // vid
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->vTitle->EditValue = $this->vTitle->CurrentValue;
