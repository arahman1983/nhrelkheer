...
$study_edit = NULL; // Initialize page object first

class cstudy_edit extends cstudy {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("studylist.php"); // Invalid key, return to list
...
// Create page object
if (!isset($study_edit)) $study_edit = new cstudy_edit();

// Page init
$study_edit->Page_Init();

// Page main
$study_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$study_edit->Page_Render();
?>
...
var CurrentForm = fstudyedit = new ew_Form("fstudyedit", "edit");

// Validate form
fstudyedit.Validate = function() {
...
fstudyedit.Form_CustomValidate = 
...
fstudyedit.ValidateRequired = true;
<?php } else { ?>
fstudyedit.ValidateRequired = false; 
<?php } ?>
...
<?php $study_edit->ShowPageHeader(); ?>
<?php
$study_edit->ShowMessage();
?>
<form name="fstudyedit" id="fstudyedit" class="<?php echo $study_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($study_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $study_edit->Token ?>">
<?php } ?>
...
<?php if ($study->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_study_id" class="col-sm-2 control-label ewLabel"><?php echo $study->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $study->id->CellAttributes() ?>>
<span id="el_study_id">
<span<?php echo $study->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $study->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="study" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($study->id->CurrentValue) ?>">
<?php echo $study->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $study_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
