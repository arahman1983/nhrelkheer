...
$study_view = NULL; // Initialize page object first

class cstudy_view extends cstudy {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "studylist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "studylist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "studylist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($study_view)) $study_view = new cstudy_view();

// Page init
$study_view->Page_Init();

// Page main
$study_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$study_view->Page_Render();
?>
...
var CurrentForm = fstudyview = new ew_Form("fstudyview", "view");

// Form_CustomValidate event
fstudyview.Form_CustomValidate = 
...
fstudyview.ValidateRequired = true;
<?php } else { ?>
fstudyview.ValidateRequired = false; 
<?php } ?>
...
<?php $study_view->ExportOptions->Render("body") ?>
<?php
	foreach ($study_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $study_view->ShowPageHeader(); ?>
<?php
$study_view->ShowMessage();
?>
<form name="fstudyview" id="fstudyview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($study_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $study_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="study">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($study->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_study_id"><?php echo $study->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $study->id->CellAttributes() ?>>
<span id="el_study_id">
<span<?php echo $study->id->ViewAttributes() ?>>
<?php echo $study->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($study->breif->Visible) { // breif ?>
	<tr id="r_breif">
		<td><span id="elh_study_breif"><?php echo $study->breif->FldCaption() ?></span></td>
		<td data-name="breif"<?php echo $study->breif->CellAttributes() ?>>
<span id="el_study_breif">
<span<?php echo $study->breif->ViewAttributes() ?>>
<?php echo $study->breif->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($study->details->Visible) { // details ?>
	<tr id="r_details">
		<td><span id="elh_study_details"><?php echo $study->details->FldCaption() ?></span></td>
		<td data-name="details"<?php echo $study->details->CellAttributes() ?>>
<span id="el_study_details">
<span<?php echo $study->details->ViewAttributes() ?>>
<?php echo $study->details->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($study->pic->Visible) { // pic ?>
	<tr id="r_pic">
		<td><span id="elh_study_pic"><?php echo $study->pic->FldCaption() ?></span></td>
		<td data-name="pic"<?php echo $study->pic->CellAttributes() ?>>
<span id="el_study_pic">
<span>
<?php echo ew_GetFileViewTag($study->pic, $study->pic->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($study->key_wrds->Visible) { // key_wrds ?>
	<tr id="r_key_wrds">
		<td><span id="elh_study_key_wrds"><?php echo $study->key_wrds->FldCaption() ?></span></td>
		<td data-name="key_wrds"<?php echo $study->key_wrds->CellAttributes() ?>>
<span id="el_study_key_wrds">
<span<?php echo $study->key_wrds->ViewAttributes() ?>>
<?php echo $study->key_wrds->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
