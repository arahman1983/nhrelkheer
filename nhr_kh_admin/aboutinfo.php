...
//
// Table class for about
//
class cabout extends cTable {
	var $id;
	var $title;
	var $breif;
	var $details;
	...
	function __construct() {
		...
		// id
		$this->id = new cField('about', 'about', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// title
		$this->title = new cField('about', 'about', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// breif
		$this->breif = new cField('about', 'about', 'x_breif', 'breif', '`breif`', '`breif`', 201, -1, FALSE, '`breif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['breif'] = &$this->breif;

		// details
		$this->details = new cField('about', 'about', 'x_details', 'details', '`details`', '`details`', 201, -1, FALSE, '`details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['details'] = &$this->details;

		// pic
		$this->pic = new cField('about', 'about', 'x_pic', 'pic', '`pic`', '`pic`', 200, -1, TRUE, '`pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['pic'] = &$this->pic;

		// key_wrds
		$this->key_wrds = new cField('about', 'about', 'x_key_wrds', 'key_wrds', '`key_wrds`', '`key_wrds`', 201, -1, FALSE, '`key_wrds`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['key_wrds'] = &$this->key_wrds;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`about`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("aboutview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("aboutview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "aboutadd.php?" . $this->UrlParm($parm);
		else
			$url = "aboutadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("aboutedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("aboutadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("aboutdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->breif->EditValue = $this->breif->CurrentValue;
		...
		$this->key_wrds->EditValue = $this->key_wrds->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->breif->Exportable) $Doc->ExportCaption($this->breif);
					if ($this->details->Exportable) $Doc->ExportCaption($this->details);
					if ($this->pic->Exportable) $Doc->ExportCaption($this->pic);
					if ($this->key_wrds->Exportable) $Doc->ExportCaption($this->key_wrds);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->title->Exportable) $Doc->ExportCaption($this->title);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->breif->Exportable) $Doc->ExportField($this->breif);
						if ($this->details->Exportable) $Doc->ExportField($this->details);
						if ($this->pic->Exportable) $Doc->ExportField($this->pic);
						if ($this->key_wrds->Exportable) $Doc->ExportField($this->key_wrds);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->title->Exportable) $Doc->ExportField($this->title);
					}
