...
$videos_library_view = NULL; // Initialize page object first

class cvideos_library_view extends cvideos_library {
	...
	// Page object name
	var $PageObjName = 'videos_library_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["vid"] <> "") {
			$this->RecKey["vid"] = $_GET["vid"];
			$KeyUrl .= "&amp;vid=" . urlencode($this->RecKey["vid"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["vid"] <> "") {
				...
				$this->RecKey["vid"] = $this->vid->QueryStringValue;
			} elseif (@$_POST["vid"] <> "") {
				$this->vid->setFormValue($_POST["vid"]);
				$this->RecKey["vid"] = $this->vid->FormValue;
			} else {
				$sReturnUrl = "videos_librarylist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "videos_librarylist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "videos_librarylist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($videos_library_view)) $videos_library_view = new cvideos_library_view();

// Page init
$videos_library_view->Page_Init();

// Page main
$videos_library_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$videos_library_view->Page_Render();
?>
...
var CurrentForm = fvideos_libraryview = new ew_Form("fvideos_libraryview", "view");

// Form_CustomValidate event
fvideos_libraryview.Form_CustomValidate = 
...
fvideos_libraryview.ValidateRequired = true;
<?php } else { ?>
fvideos_libraryview.ValidateRequired = false; 
<?php } ?>
...
<?php $videos_library_view->ExportOptions->Render("body") ?>
<?php
	foreach ($videos_library_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $videos_library_view->ShowPageHeader(); ?>
<?php
$videos_library_view->ShowMessage();
?>
<form name="fvideos_libraryview" id="fvideos_libraryview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($videos_library_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $videos_library_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="videos_library">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($videos_library->vid->Visible) { // vid ?>
	<tr id="r_vid">
		<td><span id="elh_videos_library_vid"><?php echo $videos_library->vid->FldCaption() ?></span></td>
...
<?php echo $videos_library->vid->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($videos_library->vTitle->Visible) { // vTitle ?>
	<tr id="r_vTitle">
		<td><span id="elh_videos_library_vTitle"><?php echo $videos_library->vTitle->FldCaption() ?></span></td>
...
<?php echo $videos_library->vTitle->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($videos_library->vImage->Visible) { // vImage ?>
	<tr id="r_vImage">
		<td><span id="elh_videos_library_vImage"><?php echo $videos_library->vImage->FldCaption() ?></span></td>
		<td data-name="vImage"<?php echo $videos_library->vImage->CellAttributes() ?>>
<span id="el_videos_library_vImage">
<span>
<?php echo ew_GetFileViewTag($videos_library->vImage, $videos_library->vImage->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($videos_library->vUrl->Visible) { // vUrl ?>
	<tr id="r_vUrl">
		<td><span id="elh_videos_library_vUrl"><?php echo $videos_library->vUrl->FldCaption() ?></span></td>
		<td data-name="vUrl"<?php echo $videos_library->vUrl->CellAttributes() ?>>
...
<script type="text/javascript">
fvideos_libraryview.Init();
</script>
<?php
$videos_library_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$videos_library_view->Page_Terminate();
?>
