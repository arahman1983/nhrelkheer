...
$projects_project_pic_blobview = NULL; // Initialize page object first

class cprojects_project_pic_blobview extends cprojects {
	...
	// Page object name
	var $PageObjName = 'projects_project_pic_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'projects');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('projects', 'x_project_pic');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('project_pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('project_pic')));
				if (trim(strval($this->Recordset->fields('project_pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('project_pic') . "\"");
				}
...
// Create page object
if (!isset($projects_project_pic_blobview)) $projects_project_pic_blobview = new cprojects_project_pic_blobview();

// Page init
$projects_project_pic_blobview->Page_Init();

// Page main
$projects_project_pic_blobview->Page_Main();
?>
<?php
$projects_project_pic_blobview->Page_Terminate();
?>
