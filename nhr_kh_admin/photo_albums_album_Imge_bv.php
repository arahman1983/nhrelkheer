...
$photo_albums_album_Imge_blobview = NULL; // Initialize page object first

class cphoto_albums_album_Imge_blobview extends cphoto_albums {
	...
	// Page object name
	var $PageObjName = 'photo_albums_album_Imge_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'photo_albums');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('photo_albums', 'x_album_Imge');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('album_Imge');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('album_Imge')));
				if (trim(strval($this->Recordset->fields('album_Imge'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('album_Imge') . "\"");
				}
...
// Create page object
if (!isset($photo_albums_album_Imge_blobview)) $photo_albums_album_Imge_blobview = new cphoto_albums_album_Imge_blobview();

// Page init
$photo_albums_album_Imge_blobview->Page_Init();

// Page main
$photo_albums_album_Imge_blobview->Page_Main();
?>
<?php
$photo_albums_album_Imge_blobview->Page_Terminate();
?>
