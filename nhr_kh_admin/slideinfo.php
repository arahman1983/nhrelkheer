...
class cslide extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('slide', 'slide', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// brief
		$this->brief = new cField('slide', 'slide', 'x_brief', 'brief', '`brief`', '`brief`', 201, -1, FALSE, '`brief`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['brief'] = &$this->brief;

		// slink
		$this->slink = new cField('slide', 'slide', 'x_slink', 'slink', '`slink`', '`slink`', 200, -1, FALSE, '`slink`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['slink'] = &$this->slink;

		// pic
		$this->pic = new cField('slide', 'slide', 'x_pic', 'pic', '`pic`', '`pic`', 200, -1, TRUE, '`pic`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->slink->EditValue = $this->slink->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->slink->Exportable) $Doc->ExportCaption($this->slink);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->slink->Exportable) $Doc->ExportField($this->slink);
