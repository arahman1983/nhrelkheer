...
$top_adv_pic_blobview = NULL; // Initialize page object first

class ctop_adv_pic_blobview extends ctop_adv {
	...
	// Page object name
	var $PageObjName = 'top_adv_pic_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'top_adv');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('top_adv', 'x_pic');
		...
		// SQL constructor in top_adv class, top_advinfo.php
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('pic');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('pic')));
				if (trim(strval($this->Recordset->fields('pic'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('pic') . "\"");
				}
...
// Create page object
if (!isset($top_adv_pic_blobview)) $top_adv_pic_blobview = new ctop_adv_pic_blobview();

// Page init
$top_adv_pic_blobview->Page_Init();

// Page main
$top_adv_pic_blobview->Page_Main();
?>
<?php
$top_adv_pic_blobview->Page_Terminate();
?>
