...
$sectors_add = NULL; // Initialize page object first

class csectors_add extends csectors {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "sectorsview.php")
	...
	function LoadDefaultValues() {
		$this->sector_title->CurrentValue = NULL;
		$this->sector_title->OldValue = $this->sector_title->CurrentValue;
		$this->sector_brief->CurrentValue = NULL;
		$this->sector_brief->OldValue = $this->sector_brief->CurrentValue;
		$this->sector_details->CurrentValue = NULL;
		$this->sector_details->OldValue = $this->sector_details->CurrentValue;
		$this->sector_icon->Upload->DbValue = NULL;
		$this->sector_icon->OldValue = $this->sector_icon->Upload->DbValue;
		$this->sector_icon->CurrentValue = NULL; // Clear file related field
		$this->sector_pic->Upload->DbValue = NULL;
		$this->sector_pic->OldValue = $this->sector_pic->Upload->DbValue;
		$this->sector_pic->CurrentValue = NULL; // Clear file related field
		$this->sector_keywrds->CurrentValue = NULL;
		$this->sector_keywrds->OldValue = $this->sector_keywrds->CurrentValue;
	}
	...
	function RenderRow() {
		...
		$this->sector_keywrds->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->sector_icon);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->sector_pic);
	...
	function AddRow($rsold = NULL) {
		...
		// sector_title
		$this->sector_title->SetDbValueDef($rsnew, $this->sector_title->CurrentValue, "", FALSE);

		// sector_brief
		$this->sector_brief->SetDbValueDef($rsnew, $this->sector_brief->CurrentValue, "", FALSE);

		// sector_details
		$this->sector_details->SetDbValueDef($rsnew, $this->sector_details->CurrentValue, "", FALSE);

		// sector_icon
		if (!$this->sector_icon->Upload->KeepFile) {
			$this->sector_icon->Upload->DbValue = ""; // No need to delete old file
		...
		if (!$this->sector_pic->Upload->KeepFile) {
			$this->sector_pic->Upload->DbValue = ""; // No need to delete old file
		...
		// sector_keywrds
		$this->sector_keywrds->SetDbValueDef($rsnew, $this->sector_keywrds->CurrentValue, "", FALSE);
		if (!$this->sector_icon->Upload->KeepFile) {
...
// Create page object
if (!isset($sectors_add)) $sectors_add = new csectors_add();

// Page init
$sectors_add->Page_Init();

// Page main
$sectors_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$sectors_add->Page_Render();
?>
...
var CurrentForm = fsectorsadd = new ew_Form("fsectorsadd", "add");

// Validate form
fsectorsadd.Validate = function() {
...
fsectorsadd.Form_CustomValidate = 
...
fsectorsadd.ValidateRequired = true;
<?php } else { ?>
fsectorsadd.ValidateRequired = false; 
<?php } ?>
...
<?php $sectors_add->ShowPageHeader(); ?>
<?php
$sectors_add->ShowMessage();
?>
<form name="fsectorsadd" id="fsectorsadd" class="<?php echo $sectors_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($sectors_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $sectors_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $sectors_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
