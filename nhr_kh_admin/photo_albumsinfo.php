...
//
// Table class for photo_albums
//
class cphoto_albums extends cTable {
	var $id;
	var $album_title;
	var $album_Imge;
	...
	function __construct() {
		...
		$this->TableVar = 'photo_albums';
		$this->TableName = 'photo_albums';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`photo_albums`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('photo_albums', 'photo_albums', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// album_title
		$this->album_title = new cField('photo_albums', 'photo_albums', 'x_album_title', 'album_title', '`album_title`', '`album_title`', 200, -1, FALSE, '`album_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['album_title'] = &$this->album_title;

		// album_Imge
		$this->album_Imge = new cField('photo_albums', 'photo_albums', 'x_album_Imge', 'album_Imge', '`album_Imge`', '`album_Imge`', 200, -1, TRUE, '`album_Imge`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['album_Imge'] = &$this->album_Imge;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`photo_albums`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("photo_albumsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("photo_albumsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "photo_albumsadd.php?" . $this->UrlParm($parm);
		else
			$url = "photo_albumsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("photo_albumsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("photo_albumsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("photo_albumsdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->album_title->EditValue = $this->album_title->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->album_title->Exportable) $Doc->ExportCaption($this->album_title);
					if ($this->album_Imge->Exportable) $Doc->ExportCaption($this->album_Imge);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->album_title->Exportable) $Doc->ExportCaption($this->album_title);
					if ($this->album_Imge->Exportable) $Doc->ExportCaption($this->album_Imge);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->album_title->Exportable) $Doc->ExportField($this->album_title);
						if ($this->album_Imge->Exportable) $Doc->ExportField($this->album_Imge);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->album_title->Exportable) $Doc->ExportField($this->album_title);
						if ($this->album_Imge->Exportable) $Doc->ExportField($this->album_Imge);
