...
$photo_albums_add = NULL; // Initialize page object first

class cphoto_albums_add extends cphoto_albums {
	...
	// Page object name
	var $PageObjName = 'photo_albums_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "photo_albumsview.php")
	...
	function LoadDefaultValues() {
		$this->album_title->CurrentValue = NULL;
		$this->album_title->OldValue = $this->album_title->CurrentValue;
		$this->album_Imge->Upload->DbValue = NULL;
		$this->album_Imge->OldValue = $this->album_Imge->Upload->DbValue;
		$this->album_Imge->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		$this->album_Imge->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->album_Imge);
	...
	function AddRow($rsold = NULL) {
		...
		// album_title
		$this->album_title->SetDbValueDef($rsnew, $this->album_title->CurrentValue, "", FALSE);

		// album_Imge
		if (!$this->album_Imge->Upload->KeepFile) {
			$this->album_Imge->Upload->DbValue = ""; // No need to delete old file
...
// Create page object
if (!isset($photo_albums_add)) $photo_albums_add = new cphoto_albums_add();

// Page init
$photo_albums_add->Page_Init();

// Page main
$photo_albums_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$photo_albums_add->Page_Render();
?>
...
var CurrentForm = fphoto_albumsadd = new ew_Form("fphoto_albumsadd", "add");

// Validate form
fphoto_albumsadd.Validate = function() {
...
fphoto_albumsadd.Form_CustomValidate = 
...
fphoto_albumsadd.ValidateRequired = true;
<?php } else { ?>
fphoto_albumsadd.ValidateRequired = false; 
<?php } ?>
...
<?php $photo_albums_add->ShowPageHeader(); ?>
<?php
$photo_albums_add->ShowMessage();
?>
<form name="fphoto_albumsadd" id="fphoto_albumsadd" class="<?php echo $photo_albums_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($photo_albums_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $photo_albums_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $photo_albums_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fphoto_albumsadd.Init();
</script>
<?php
$photo_albums_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$photo_albums_add->Page_Terminate();
?>
