...
class cphotos extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('photos', 'photos', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// title
		$this->title = new cField('photos', 'photos', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// album_name
		$this->album_name = new cField('photos', 'photos', 'x_album_name', 'album_name', '`album_name`', '`album_name`', 200, -1, FALSE, '`album_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->fields['album_name'] = &$this->album_name;

		// images
		$this->images = new cField('photos', 'photos', 'x_images', 'images', '`images`', '`images`', 201, -1, TRUE, '`images`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`photos`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("photosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("photosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "photosadd.php?" . $this->UrlParm($parm);
		else
			$url = "photosadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("photosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("photosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->album_name->Exportable) $Doc->ExportCaption($this->album_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->album_name->Exportable) $Doc->ExportField($this->album_name);
