...
class coffers_edit extends coffers {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("offerslist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		$this->offers_keywrds->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->offers_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// offers_title
			$this->offers_title->SetDbValueDef($rsnew, $this->offers_title->CurrentValue, "", $this->offers_title->ReadOnly);

			// offers_brief
			$this->offers_brief->SetDbValueDef($rsnew, $this->offers_brief->CurrentValue, "", $this->offers_brief->ReadOnly);

			// offers_details
			$this->offers_details->SetDbValueDef($rsnew, $this->offers_details->CurrentValue, "", $this->offers_details->ReadOnly);

			// offers_pic
			if (!($this->offers_pic->ReadOnly) && !$this->offers_pic->Upload->KeepFile) {
				$this->offers_pic->Upload->DbValue = $rsold['offers_pic']; // Get original value
			...
			// project
			$this->project->SetDbValueDef($rsnew, $this->project->CurrentValue, "", $this->project->ReadOnly);

			// offers_keywrds
			$this->offers_keywrds->SetDbValueDef($rsnew, $this->offers_keywrds->CurrentValue, "", $this->offers_keywrds->ReadOnly);
...
foffersedit.Lists["x_project"] = {"LinkField":"x_project_title","Ajax":true,"AutoFill":false,"DisplayFields":["x_project_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if (@$_POST["fa_x_offers_pic"] == "0") { ?>
<input type="hidden" name="fa_x_offers_pic" id= "fa_x_offers_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_offers_pic" id= "fa_x_offers_pic" value="1">
<?php } ?>
